...
class TWOSTAGECARTOONIZATIONModel(BaseModel):
    def name(self):
        return 'TWOSTAGECARTOONIZATIONModel'
    ...
    def initialize(self, opt):
        ...
        self.netG_A = networks.define_C(opt.input_nc, opt.output_nc, opt.ngf, opt.netGA, opt.norm, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def backward_D_basic5(self, netD, real, real2, real3, real4, real5, fake):
        ...
        # # Combined loss * 2
        loss_D = (loss_D_real + loss_D_real2 + loss_D_real3 + loss_D_real4 + loss_D_real5) * 0.1 + loss_D_fake * 0.5
        loss_D.backward()
        return loss_D
    ...
    def backward_G_first(self):
        ...
        if self.opt.identity_A > 0.0:
            self.loss_idt_A = self.criterionL1(self.idt_A, self.real_BS) * self.opt.identity_A 
        else:
...
#         self.loss_fla = Flattening_Loss2_Gause(self.device, self.real_A, self.RGA, self.gause_radius, 0.2, 1, self.Kernel)*1.5
        self.loss_fla = torch.tensor([0.]).to(self.device)
        self.loss_GDA = self.criterionGAN(self.netD_A(self.RGA), True)
        self.loss_G_A = self.loss_conA + self.loss_idt_A + self.loss_fla + self.loss_GDA
    ...
    def backward_G_third(self):
        ...
        if self.opt.identity_L > 0.0:
            self.loss_idt_L = self.criterionL1(self.idt_L, self.real_B) * self.opt.identity_L 
        else:
            self.loss_idt_L = torch.tensor([0.]).to(self.device)
